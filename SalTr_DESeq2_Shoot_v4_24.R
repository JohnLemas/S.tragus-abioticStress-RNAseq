...
cts <- cts[,c(1:3,25:27,31:33)]

# Subset the metadata for the analysis:
metadata1 <- metadata_full[c(1:3,25:27,31:33),]
...
# Save for reference:
pdf("03_output/05_DESeq2/Figure_files/Shoot/dispEst.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
write(counts_norm, file = "03_output/05_DESeq2/03_cts_files/Shoot_24_cts_norm.txt", sep = "\t")
write(counts_nonNorm, file = "03_output/05_DESeq2/03_cts_files/Shoot_24_cts_nonNorm.txt", sep = "\t")
...
pdf("03_output/05_DESeq2/Figure_files/Shoot/total_counts.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/Shoot/library_complx.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
# Save for reference:
pdf("03_output/05_DESeq2/Figure_files/Shoot/sdVmean_normTrans.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
# Save for reference:
pdf("03_output/05_DESeq2/Figure_files/Shoot/sdVmean.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
plot_pca(vsd, PC_x = 1, PC_y = 2, color_by = "time", shape_by = "trt")

pca <- plot_pca(vsd, PC_x = 1, PC_y = 2, color_by = "time", shape_by = "trt")

pdf("03_output/05_DESeq2/Figure_files/Shoot/PCA.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
write.csv(x = loadings, file = "03_output/05_DESeq2/Figure_files/Shoot/PCA_loadings.csv")
...
pdf("03_output/05_DESeq2/Figure_files/Shoot/heatmap.pdf", height = 6, width = 7)
par(mfrow=c(1,1))
p
...
pdf("03_output/05_DESeq2/Figure_files/Shoot/Antiporter.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
ggplot(data = antiporter_table, mapping = aes(
  ...
  labs(title = "Salt treated shoot tissue", subtitle = "SalTrChr04Ag088810\nNHX1: Vacuolar Na+/H+ Exchanger",
       caption = "Tukey djusted pairwise comparison: Contl - NaCl: p = 0.004; Contl - Cold: p < 0.001;\nNaCl - Cold: p < 0.001",
       color = "Hours after\ntreatment")
...
pdf(file = "03_output/05_DESeq2/Figure_files/Shoot/NHX1_boxplot.pdf", width = 6, height = 6)
...
# CAX1 Calcium vacuolar antiporter
plotCounts(dds, gene=which(rownames(res_0_24) == "SalTrChr07Ag163170"), intgroup=c("time", "trt"))

antiporter_countsplot <- plotCounts(dds, gene=which(rownames(res_0_24) == "SalTrChr07Ag163170"), intgroup=c("time", "trt"))

antiporter_counts_table <- plotCounts(dds, gene=which(rownames(res_0_24) == "SalTrChr07Ag163170"), intgroup=c("time", "trt"), returnData = TRUE)
...
ggplot(data = antiporter_table, mapping = aes(
  ...
  labs(title = "Salt treated shoot tissue", subtitle = "SalTrChr07Ag163170\nCAX1: Vacuolar Ca2+/H+ Exchanger",
       caption = "Tukey adjusted pairwise comparison: Contl - NaCl: p = 0.113; Contl - Cold: p = 0.127;\nNaCl - Cold: p = 0.996")
...
pdf(file = "03_output/05_DESeq2/Figure_files/Shoot/CAX1_boxplot.pdf", width = 6, height = 6)
...
# Gene: SalTrChr03Ag060690

pdf("03_output/05_DESeq2/Figure_files/Shoot/padj_min_dotplot.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
write(rownames(Up_0_24), file = "03_output/05_DESeq2/Up_shoot_0_24.txt", sep = "\n")
write(rownames(Down_0_24), file = "03_output/05_DESeq2/Down_shoot_0_24.txt", sep = "\n")
write(rownames(res_0_24), file = "03_output/05_DESeq2/All_shoot_0_24.txt", sep = "\n")

###########################################

pdf("03_output/05_DESeq2/Figure_files/Shoot/Evolcano_shoot_0_24.pdf",
    width = 8, height = 8)
...
EnhancedVolcano(toptable = res_0_24, lab = rownames(res_0_24), 
                ...
                subtitle = 'B: Cold and salt treated shoot tissue samples',
                caption = bquote(~Log[2]~ "fold change cutoff = ±2; p-value cutoff: p < 0.05"),
                xlab = bquote(~Log[2]~ 'fold change'),
                pCutoff = 5e-2,
                selectLab = c("SalTrChr03Ag060690",
                              "SalTrChr07Ag163470",
                              "SalTrChr02Bg246160",
                              "SalTrChr07Ag163470",
                              "SalTrChr03Bg279880",
