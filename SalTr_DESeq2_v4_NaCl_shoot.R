...
cts_contrast <- cts[,c(1:3, 7:9, 19:21, 31:33)]
...
metadata_contrast <- metadata_full[c(1:3, 7:9, 19:21, 31:33),]
...
# Alter the below factor levels based on what the design for this run is:
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/dispEst.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/total_counts.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
write.table(counts_norm, file = "03_output/05_DESeq2/03_cts_files/NaCl_shoot_cts_norm.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)
write.table(counts_nonNorm, file = "03_output/05_DESeq2/03_cts_files/NaCl_shoot_cts_nonNorm.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)

# Lets also save the summary of the normalized counts for reference:

counts_sum <- summary(counts(dds, normalized = TRUE))

write.table(counts_sum, file = "03_output/05_DESeq2/03_cts_files/NaCl_shoot_cts_norm_summary.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)

#' upload these tables for supplimental information in the manuscript
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/sdVmean_normTrans.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/sdVmean.pdf", height = 8, width = 7)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/PCA.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
write.csv(x = loadings, file = "03_output/05_DESeq2/Figure_files/NaCl_shoot/PCA_loadings.csv")
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/heatmap.pdf", height = 6, width = 7)
par(mfrow=c(1,1))
p
...
# exclude any genes as significant because their fold change is too low. Default is 0. That means, 
# take all statistically significant genes. In this dataset, I selected 0.5 because there were
# A LOT of significant genes and I'd like to reduce the number.
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/MAplots.pdf", height = 8, width = 8)
...
write.table(antiporter_counts_table, file = "03_output/05_DESeq2/03_cts_files/antiporter_counts_table.txt",
            row.names = TRUE, col.names = TRUE)
...
ggplot(data = antiporter_table, mapping = aes(
  ...
  labs(title = "Salt treated shoot tissue", subtitle = "SalTrChr04Ag088810\nNHX1: Vacuolar Na+/H+ Exchanger",
       caption = "Tukey adjusted pairwise comparison: 0 - 24 HAT: p = 0.004; 0 - 3 HAT: p = 0.001;\n0 - 8 HAT: p < 0.001")
...
pdf(file = "03_output/05_DESeq2/Figure_files/NaCl_shoot/NHX1_boxplot.pdf", width = 6, height = 6)
...
############### CAX1
plotCounts(dds, gene=which(rownames(res_0_8) == "SalTrChr07Ag163170"), intgroup=c("trt", "time"))

antiporter_countsplot <- plotCounts(dds, gene=which(rownames(res_0_8) == "SalTrChr07Ag163170"), intgroup=c("trt", "time"))

antiporter_counts_table <- plotCounts(dds, gene=which(rownames(res_0_8) == "SalTrChr07Ag163170"), intgroup=c("trt", "time"), returnData = TRUE)
...
ggplot(data = antiporter_table, mapping = aes(
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        plot.caption = element_text(hjust = 0.01),
        text = element_text(family = "Times New Roman", size = 12)) +
  labs(title = "Salt treated shoot tissue", subtitle = "SalTrChr07Ag163170\nCAX1: Vacuolar Ca2+/H+ Exchanger",
       caption = "Tukey adjusted pairwise comparison: 0 - 24 HAT: p = 0.019; 0 - 3 HAT: p = 0.311;\n0 - 8 HAT: p = 0.494")
...
pdf(file = "03_output/05_DESeq2/Figure_files/NaCl_shoot/CAX1_boxplot.pdf", width = 6, height = 6)
...
########### Aquaporin intermembrane transporters IPR000425:

############ make a boxplot and save it!

aquaporin_countsplot <- plotCounts(dds, gene=which(rownames(res_0_24) == "SalTrChr02Ag027890"), intgroup = "time")

aquaporin_counts_table <- plotCounts(dds, gene=which(rownames(res_0_24) == "SalTrChr02Ag027890"), intgroup="time", returnData = TRUE)

aquaporin_table <- data.frame(sample = metadata_contrast$names1,
                              count = aquaporin_counts_table$count,
                              time = aquaporin_counts_table$time)

ggplot(data = aquaporin_table) + 
  geom_point(mapping = aes(
    x = time, y = count)) +
  theme_light() + ggtitle("Aquaporin intermembrane transporters") +
  xlab("Hours after treatment") + ylab("Normalized gene count") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        plot.caption = element_text(hjust = 0.01),
        text = element_text(family = "Times New Roman", size = 12)) +
  labs(title = "All tissues 24 HAT", subtitle = "SalTrChr02Ag027890\nAquaporin intermembrane transporters",
       caption = "Tukey djusted pairwise comparison: 0 - 3 HAT: p = 0.006; 0 - 8 HAT: p < 0.001;\n0 - 24 HAT: p < 0.155")

# save this gene count plot

antiporter_boxplot <- ggplot(data = aquaporin_table) + 
  geom_point(mapping = aes(
    x = time, y = count)) +
  theme_light() + ggtitle("Aquaporin intermembrane transporters") +
  xlab("Hours after treatment") + ylab("Normalized gene count") +
...
pdf(file = "03_output/05_DESeq2/Figure_files/NaCl_shoot/aquaporin_boxplot.pdf", width = 6, height = 6)
...
aquaporin_anova <- anova(lm(count ~ time, data = aquaporin_table))

aquaporin_emmeans <- emmeans(lm(count ~ time, data = aquaporin_table),
                              ~ time)

aquaporin_pairs <- pairs(
  aquaporin_emmeans, adjust = "Tukey"
)
...
plotCounts(dds, gene=which.min(res_0_3$padj), intgroup="time")

plotCounts(dds, gene=which.min(res_0_8$padj), intgroup="time")
...
pdf(file = "03_output/05_DESeq2/Figure_files/NaCl_shoot/antiporter_dotplot.pdf", width = 6, height = 6)
...
plot_gene("SalTrChr02Ag029430", dds, x_var = "time", color_by = "rep")
...
write(rownames(Up_0_3), file = "03_output/05_DESeq2/Up_shoot_NaCl_0_3.txt", sep = "\n")
write(rownames(Down_0_3), file = "03_output/05_DESeq2/Down_shoot_NaCl_0_3.txt", sep = "\n")
write(rownames(res_0_3), file = "03_output/05_DESeq2/All_shoot_NaCl_0_3.txt", sep = "\n")
...
# Save these lists to output files:
write(rownames(Up_0_8), file = "03_output/05_DESeq2/Up_shoot_NaCl_0_8.txt", sep = "\n")
write(rownames(Down_0_8), file = "03_output/05_DESeq2/Down_shoot_NaCl_0_8.txt", sep = "\n")
write(rownames(res_0_8), file = "03_output/05_DESeq2/All_shoot_NaCl_0_8.txt", sep = "\n")
write(rownames(lowestLFC_0_8), file = "03_output/05_DESeq2/lowestLFC_shoot_NaCl_0_8.txt", sep = "\n")
...
# Save these lists to output files:
write(rownames(Up_0_24), file = "03_output/05_DESeq2/Up_shoot_NaCl_0_24.txt", sep = "\n")
write(rownames(Down_0_24), file = "03_output/05_DESeq2/Down_shoot_NaCl_0_24.txt", sep = "\n")
write(rownames(res_0_24), file = "03_output/05_DESeq2/All_shoot_NaCl_0_24.txt", sep = "\n")
...
pdf("03_output/05_DESeq2/Figure_files/NaCl_shoot/clustered_heatmap.pdf", width = 6, height = 8)
