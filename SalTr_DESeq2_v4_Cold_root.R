...
cts_contrast <- cts[,c(4:6, 16:18, 28:30)]
...
metadata_contrast <- metadata_full[c(4:6, 16:18, 28:30),]
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/dispEst.pdf", height = 6, width = 8)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/total_counts.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/library_complx.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
write.table(counts_norm, file = "03_output/05_DESeq2/03_cts_files/Cold_root_cts_norm.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)
write.table(counts_nonNorm, file = "03_output/05_DESeq2/03_cts_files/Cold_root_cts_nonNorm.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)

# Lets do the summary table as well:

counts_sum <- summary(counts(dds, normalized = TRUE))
write.table(counts_sum, file = "03_output/05_DESeq2/03_cts_files/Cold_root_cts_norm_summary.txt", sep = "\t",
            row.names = TRUE, col.names = TRUE)
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/sdVmean_normTrans.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/sdVmean.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/PCA.pdf", height = 7, width = 7)
par(mfrow=c(1,1))
...
write.csv(x = loadings, file = "03_output/05_DESeq2/Figure_files/Cold_root/PCA_loadings.csv")
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/heatmap.pdf", height = 6, width = 7)
par(mfrow=c(1,1))
p
...
# Set your lfcThreshold to a value of your choosing. This will set whether you want to exclude 
# any genes as significant because their fold change is too low. Default is 0. That means, take all 
# statistically significant genes. In this dataset, I selected 0.5 because there were 
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/MAplots.pdf", height = 6, width = 8)
...
ggplot(data = antiporter_table, mapping = aes(
  ...
  labs(title = "Cold treated root tissue", subtitle = "SalTrChr04Ag088810\nNHX1: Vacuolar Na+/H+ Exchanger",
       caption = "Tukey adjusted pairwise comparison: 0 - 24 HAT: p = 0.001; 0 - 6 HAT: p = 0.895")
...
pdf(file = "03_output/05_DESeq2/Figure_files/Cold_root/NHX1_boxplot.pdf", width = 6, height = 6)
...
pdf(file = "03_output/05_DESeq2/Figure_files/Cold_root/antiporter_dotplot.pdf", width = 6, height = 6)
...
write(rownames(Up_0_6), file = "03_output/05_DESeq2/Up_root_Cold_0_6.txt", sep = "\n")
write(rownames(Down_0_6), file = "03_output/05_DESeq2/Down_root_Cold_0_6.txt", sep = "\n")
write(rownames(res_0_6), file = "03_output/05_DESeq2/All_root_Cold_0_6.txt", sep = "\n")
...
lowestLFC_0_24 <- subset(res_0_24, log2FoldChange < -20)
lowestLFC_0_24 <- lowestLFC_0_24[order(lowestLFC_0_24$log2FoldChange, decreasing = FALSE),]

head(Down_0_24)
dim(Down_0_24)

# Save these lists to output files:
write(rownames(Up_0_24), file = "03_output/05_DESeq2/Up_root_Cold_0_24.txt", sep = "\n")
write(rownames(Down_0_24), file = "03_output/05_DESeq2/Down_root_Cold_0_24.txt", sep = "\n")
write(rownames(res_0_24), file = "03_output/05_DESeq2/All_root_Cold_0_24.txt", sep = "\n")
write(rownames(lowestLFC_0_24), file = "03_output/05_DESeq2/lowestLFC_root_Cold_0_24.txt", sep = "\n")
...
# Draw a heat map
# Scale by row, listed below as scale = "row". This is really important. 
# It sets the mean of every row to 0 and the standard deviation of every row to 1:
...
pdf("03_output/05_DESeq2/Figure_files/Cold_root/clustered_heatmap.pdf", width = 6, height = 8)
